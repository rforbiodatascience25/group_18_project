...
```{r}
#first the bar
make_family_bar <- function(df, gene_order, family_colors) {

  df_unique <- df |> 
    distinct(gene, family) |> 
    mutate(gene = factor(gene, levels = gene_order))

  ggplot(df_unique, aes(x = 1, y = gene, fill = family)) +
    geom_tile() +
    scale_fill_manual(values = family_colors, name = "HDAC family") +
...
family_bar_os <- make_family_bar(df_os, gene_order, family_colors)
family_bar_dfs <- make_family_bar(df_dfs, gene_order, family_colors)
...
```{r}
#then the legend
#data frame with families
family_legend_df <- data.frame(
  family = factor(
    c("class I", "class IIA", "class IIB", "class III", "class IV"),
    levels = c("class I", "class IIA", "class IIB", "class III", "class IV")
  )
)

#small ggplot
family_legend_plot <- ggplot(family_legend_df, aes(x = family, y = 1, fill = family)) +
  geom_tile() +
  scale_fill_manual(values = family_colors, name = "HDAC family") +
  theme_void() +
  theme(
...
#extract it
family_legend <- get_legend(family_legend_plot)
family_legend_wrap <- wrap_elements(full = family_legend)
...
pval_leg_plot <- ggplot(pval_leg_df, aes(x = 1, y = p_shape, shape = p_shape)) +
  geom_point(size = 3, color = "black") +
  scale_shape_manual(
    values = c(circle = 16, square = 15, triangle = 17),
...
hr_leg_plot <- ggplot(hr_leg_df, aes(x, y, fill = val)) +
  geom_tile() +
...
```{r}
plot_os_test <- p_os + hr_legend_wrap +
...
```{r}
plot_os <- family_bar_os + p_os +
  plot_layout(widths = c(0.1, 1))

plot_dfs <- family_bar_dfs + p_dfs + 
  plot_layout(widths = c(0.1, 1))
...
final_plot_os <- (
  ...
  plot_layout(widths = c(0.07, 1.6, 0.06, 0.35))
