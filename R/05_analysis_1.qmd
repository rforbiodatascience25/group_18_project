...
#converting descriptive text values to numierc values to enable statistical calculations
...
#next step: filtering the data, leaving only the selected cancer types and arranging them in order on the graph
cancer_order <- c(
  "LIHC", "UCEC", "KIRC", "LGG",
  "OV", "PAAD",
  "HNSC", "ESCA", "CESC", "STAD",
...
df_long$label[df_long$label == ""] <- NA
...
p_annot <- ggplot(annot_df, aes(
  ...
  guides(fill = guide_legend(title = "HDAC family", order = 1))
...
p_heatmap <- ggplot(df_long,
  ...
  geom_point(aes(shape = label), size = 0, alpha = 0)+ 
  geom_text(aes(label=label), size=3, vjust = 0.75) +
  scale_fill_gradientn(colors=my_colours,
                       limits=c(-0.3,0.3),
                       name = "rho value",
                       guide = guide_colorbar(order = 2, barwidth = 0.4, barheight = 3)) + 
  scale_shape_manual(
    name = "p value",
    values = c("•", "▲", "■"),
  ...
  guides(
    shape = guide_legend(override.aes = list(alpha = 1, size = 2),order = 3))
...
```{r}
#here we define genes in the order for the graph
...
p_heatmap_stage <- ggplot(df_long_stage,
                          aes(x=Cancer,y=factor(gene, levels=rev(final_genes_stage)), fill=value)) +
  geom_tile(color = "white", size = 0.2) +
  geom_point(aes(shape = label), size = 0, alpha = 0) +
  geom_text(aes(label=label), size=3, vjust = 0.75) +
  scale_fill_gradientn(colors=my_colours_stage, 
                       limits=c(-0.3,0.3),
                       name = "rho value",
                       guide = guide_colorbar(order = 2, barwidth = 0.4, barheight = 3)) + 
  ...
  guides(shape = guide_legend(override.aes = list(alpha = 1, size = 2), order = 3)
  ) +
